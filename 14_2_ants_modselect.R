#' ---
#' title: "Ant data: model selection"
#' author: Brett Melbourne
#' date: 12 Dec 2018 (updated 20 Nov 2021)
#' output:
#'     github_document:
#'         pandoc_args: --webtex
#' ---

#' Fourth in a series of scripts to analyze the ant data described in Ellison
#' (2004). This script considers model selection from several perspectives,
#' including frequentist, Bayesian, information theoretic, and predictionist.
#'
...
#' Fit the basic model from our previous investigations. We previously found
#' that a model with overdispersion could not be fitted by the frequentist
#' `glmer()` algorithm because the overdispersion variance was close to zero, so
#' our best recourse is to leave it out. The Bayesian algorithm did not have a
#' problem estimating this variance. We'll fit all of these versions for later
#' comparison. Also, we needed to scale the predictor variables to get the
#' frequentist algorithm to work, so we'll work with the scaled variables to
#' enable comparison between frequentist and Bayesian (this does not affect the
#' inferences).
...
#' asking the question: Should the model include an interaction of habitat and
#' latitude?
#' 
...
#' reported for models fitted by maximum likelihood. The deviance is minus twice
#' the log-likelihood, so the Chi-squared test statistic is equivalently the
#' difference in deviance of the models. In the following table `Chisq` is the
#' Chi-squared statistic and its associated p-value is shown.
...
#' it out. Although the above asymptotic test is frequently used for multilevel
#' models, it's not entirely clear that it is theoretically valid. The most
#' defensible approach here would be a bootstrapped p-value. See code in
#' [04_5_bootstrap_p-value.md](04_5_bootstrap_p-value.md).
#' 
...
#' stable approximation to the HPDI since the posterior is symmetric. We'll use
#' the overdispersed model because the Bayesian algorithm has no problem
#' fitting it.

summary(bayes_odHxL, pars="beta", probs=c(0.025,0.975), digits=4)

#' Since zero is well within the credible interval, we could decide to leave the
#' interaction out of the model.
#'
#' Of course, we could come to the same decision from the frequentist confidence
#' interval for the interaction, which is almost the same as the Bayesian
#' credible interval (for comparison, given here for the Bayesian model without
#' overdispersion):
...
#' coefficient, cross validation focuses on the predictive performance of the
#' model on new data (in other words, out-of-sample performance). Since CV is a
#' general tool, we can apply it to either the likelihood or Bayesian analysis.
#' Here, I'll illustrate CV with the likelihood fit, since it is faster. We'll
#' do leave-one-out-CV (LOOCV), which is a special case of k-fold CV where k =
#' n. LOOCV is a good choice for the multilevel model because we won't make any
...
#' leave-one-out information criterion (LOOIC). The measure of out-of-sample
#' predictive fit for LOOIC is the expected log predictive density:
#'
...
#'
#' It would be very computationally expensive to do LOOCV directly on Bayesian
#' models (it would require refitting the model leaving out each observation!).
#' The LOOIC approach approximates this for most points but uses a direct CV
#' when the approximation is not good (typically zero to a few points).
#' 

#' As we're able to fit the overdispersed Bayesian model just fine, we'll do
#' that here. First fit the model without the interaction term:
...
#' the interaction (this will take a minute or so):

loo(bayes_odHL)

#' This reports the leave-one-out expected log predictive density (`elpd_loo`),
#' the effective number of parameters (`p_loo`), and the leave-one-out
#' information criterion (`looic`), which is `-2 * elpd_loo`. There are also
#' some diagnostics for this algorithm. We are hoping to see that all Pareto k
#' estimates are at least `ok`. If not, there will be a warning message that
#' some observations had a pareto_k > 0.7, which is telling us that the
#' approximation might not be accurate for those observations. We would then
#' follow any advice here to set the `k_threshold` argument as follows, which
#' will perform a direct LOOCV simulation for those observations. In practice, I
#' find this happens often enough that I always set `k_threshold` by default.
...
#' interaction has lower LOOIC (see the lines above labelled `looic`), so we
#' favor the simpler model without the interaction.
#' 
...
#' then we might fall back to maximum likelihood and AICc, provided those models
#' can converge. Here we are including the overdispersion term (`plot`) in all
#' the models (and dropping `od` from the object name to simplify the code).
#' 
...
#' `bayesHLE`, the model with each of the variables but with no interactions. We
#' have also calculated the LOOIC differences (and their standard errors) from
#' the best model. A LOOIC difference of about 2 indicates a fairly negligible
#' difference between models. So, the top four models all have about the same
#' predictive performance. Also notice the uncertainty in these differences is
#' reasonably large for small differences. By default, `loo_compare()` reports
#' results for the expected log predictive density (ELPD). LOOIC is -2*ELPD and
#' we calculated that here instead for consistency with the AIC convention.
#'
