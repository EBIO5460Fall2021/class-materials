...
date: 11 Nov 2018 (updated 2 Dec 2021)
output:
    github_document:
        pandoc_args: --webtex
---

Simulating a model means to create data using the model. Such "data" are called fake data, or simulated data. Why simulate a model?
...
4. **After** you've done an analysis, simulation can be used to ask: **"Does the fitted model look right?"**. In other words, does the fitted model generate data that look anything like the real data? In `rstanarm` this is the **posterior predictive check** in the diagnostics.
...
We have looked at how to simulate a multilevel model. The simulation algorithm is straightforward. Simulation allows us to check that everything is working or to conduct detailed investigations of training or inference algorithms.
