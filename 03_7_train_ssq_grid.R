...
#' Generate some fake data to work with. Your algorithm will use your own dataset.
...
#' Here is an algorithm to fit the model to the data by grid search. We need to
#' choose a sensible range of parameter values to search over. We can inspect
#' the data to make some good guesses. The y-intercept $\beta_0$ looks like it
#' will be around 25, probably larger than 10 and less than 50. The slope
#' $\beta_1$ is rise over run, so it will be about 250/25 = 10, so we'll try
#' spanning that value.
...
#' closely at the smallest SSQ values (the underside of this plot), so we want
...
#' For example, if we look vertically above b_0 = 30, we see the SSQ for b_0 =
#' 30 with different values of b_1 (although we can't tell from this plot what
#' the values of b_1 are).
#'
#' These plots are telling us that we've found the basin that includes the
#' best-fit parameter values. From here, we could adjust the search ranges and
#' search increments of b_0 and b_1 and redo the search to give whatever number
#' of significant digits we desire. Finally, these profiles are beginning to
