...
#' date: 14 Oct 2018 (updated 20 Oct 2021)
#' output: github_document
#' ---

#' This script can be rendered to a reproducible report.\
#' `rmarkdown::render("10_3_radon_multilevel.R")`, or *Ctrl+Shift+K* in RStudio.\
#'
...
#' and does not have any fixed effects). In part II we will consider a
#' house-level (G&H 12.3-4) predictor of radon. In part III, we will consider a
...
#' No, that's terrible! That plot was a dud because the choice of a rectangle as
#' the primitive geom did not allow for clear comparison of distance on a common
#' scale. The counties are all jumbled and hard to distinguish. No worries, we
#' can try a different view to show the data more clearly. A density plot better
#' discriminates among counties and confirms our impression from above.
lrgst5cty %>%
    ggplot() +
...
#' And now for the whole dataset (all counties combined) split by floor. It is
#' simple to modify the above code to group by floor instead.
sm_byfloor <-
    radon_dat %>%
    group_by(floor) %>%
    summarize(mean=mean(log_radon),sd=sd(log_radon),min=min(log_radon),max=max(log_radon))
...
#' I'm not sure what the 86th b parameter is (we could look in the source to
#' figure it out) but the first 85 are the county samples.
#'
