...
#' date: 28 Oct 2018 (updated 28 Oct 2021)
#' output:
#'     github_document:
#'         pandoc_args: --webtex
#' ---
...
#' The estimates for the county intercepts (which is the estimated ln(radon)
#' level in a house basement) are shrunk toward the county-scale mean (dashed
#' blue line). The county-scale mean is the mean intercept among counties
#' estimated by the multilevel model. Notice that the county mean is higher than
#' the complete pooling intercept (solid blue line). This is because the
#' complete pooling intercept is influenced by the six or so most-sampled
...
#' are the number of houses in a county (not the number of sampled houses in our
#' dataset but the actual number of houses, for which we'd need to obtain
#' another dataset). Counties with more houses contribute more to the mean radon
#' level in Minnesota houses. The county-scale mean is not the "grand mean" or
#' the mean for Minnesota houses, it is the mean among Minnesota counties.
#' 
